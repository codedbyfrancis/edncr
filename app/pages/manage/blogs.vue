...
<template>
  <section v-if="profileData?.role === 'manager' || profileData?.role === 'superuser' || profileData?.role === 'editor'">
    <div class="container mx-auto px-4 py-8">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Manage Blogs</h1>
        <UButton 
          color="primary" 
          size="lg"
      ...
      <div v-if="blogsData && blogsData.length > 0" class="space-y-4">
        <UCard v-for="blog in blogsData" :key="blog.id" class="hover:shadow-lg transition-shadow">
          <div class="flex justify-between items-start">
            ...
            <div class="flex items-center gap-2 ml-4">
              ...
              <UButton 
                v-if="profileData?.role === 'manager' || profileData?.role === 'superuser' || blog.user_id === user.sub"
                color="primary" 
                variant="ghost" 
                size="sm"
              ...
              <UDropdown
                ...
                :items="[
                  [
                    { label: 'Approve', icon: 'i-heroicons-check-circle', disabled: blog.status === 'approved' },
                    { label: 'Request Changes', icon: 'i-heroicons-exclamation-triangle' },
